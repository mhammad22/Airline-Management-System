...
void line(int x1, int y1, int x2, int y2, int color) //use three 3 integers if you want colored lines.
{
...
void myCurs(int x1, int y1, int x2, int y2, int R, int G, int B, int FR, int FG, int FB)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	int mid1 = (y1 + y2) / 2;
	MoveToEx(device_context, mid, y1, NULL);
	LineTo(device_context, x2, x1);
	LineTo(device_context, mid, mid1);
	LineTo(device_context, mid + 1, y1);
	HBRUSH brush = ::CreateSolidBrush(RGB(FR, FG, FB));  //Fill color is black
	SelectObject(device_context, brush);
	FloodFill(device_context, mid, (y1 + y2) / 2, RGB(R, G, B));
...
void myDiamond(int x1, int y1, int x2, int y2, int R, int G, int B, int FR, int FG, int FB)
{
	...
	HBRUSH brush = ::CreateSolidBrush(RGB(FR, FG, FB));  //Fill color is black
	SelectObject(device_context, brush);
	FloodFill(device_context, mid1, (y1 + y2) / 2, RGB(R, G, B));
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);

}
void myHex(int x1, int y1, int x2, int y2, int R, int G, int B, int FR, int FG, int FB)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	MoveToEx(device_context, mid1, y1, NULL);
	LineTo(device_context, x2, mid2);
	LineTo(device_context, x2, mid2 + 10);
	LineTo(device_context, mid1, y2 + 10);
	LineTo(device_context, x1, mid2 + 10);
	LineTo(device_context, x1, mid2);
	LineTo(device_context, mid1, y1);
	HBRUSH brush = ::CreateSolidBrush(RGB(FR, FG, FB));  //Fill color is black
	SelectObject(device_context, brush);
...
void rectangle(int x1, int y1, int x2, int y2, int R, int G, int B, int FR, int FG, int FB)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
...
void ellipse(int x1, int y1, int x2, int y2, int R, int G, int B, int FR, int FG, int FB)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
